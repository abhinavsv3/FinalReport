...
\par In this test we explore the special JSON object with directed and weighted edges. The test also shows an instance of controlling edge thinkness, node diameter and node thickness. Figure ~\ref{test4} shows an example of this test.
\begin{figure}[H]
\centering
...
\item \textit{Network}
\par A test in which a hairball network is generated having roots and nodes.  Figure ~\ref{test5} shows an example of seven minutes of communication between members of the AngularJS, EmberJS, and KnockoutJS IRC channels.
...
\par A test in which a graph with relatedness was generated. Figure ~\ref{test6a} shows a sample having relatedness between philosophers. Figure ~\ref{test6b} shows a zoomed view of one of the philosopher graph.
...
\par In this test the difference in node are and assignment of colors to nodes was tested. Figure ~\ref{test7} presents can example in which the test is performed.
...
Alchemy.js works well with firefox than with google chrome. It was found when rendering of edges.
Bug number : 472 (https://github.com/GraphAlchemist/Alchemy/issues/472)
